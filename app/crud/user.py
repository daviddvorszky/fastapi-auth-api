...
class CRUDUser:
    ...
    async def get_user_by_username_or_email(self, db: AsyncSession, username: str, email: str):
        result = await db.execute(select(User).filter(or_(User.username.like(username), User.email.like(email))))
        return result.scalars().first()
    ...
    async def authenticate(self, db: AsyncSession, username: str, password: str):
        user = await self.get_user_by_username_or_email(db, username=username, email=username)
        if not user:
            return None
